...
from PyQt6.QtWidgets import QComboBox, QHBoxLayout, QLabel, QTabWidget, QVBoxLayout, QWidget
...
class CreateImageFromPromptTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        title.setStyleSheet('font-weight: 600; color: #f8fafc;')
...
class CreateImageTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None, config, on_model_changed):
        super().__init__(parent)
        self._cfg = config
        ...
        if idx >= 0:
            ...
            self.model_combo.setCurrentIndex(len(self.MODEL_OPTIONS) - 1)
    ...
    def get_reference_data(self) -> tuple[list[str], list[dict]]:
        prompts = []
        if hasattr(self.tab_reference, 'get_prompts'):
            prompts = self.tab_reference.get_prompts()
            
        characters = []
        if hasattr(self.tab_reference, 'get_character_items'):
            characters = self.tab_reference.get_character_items()
            
        clean_prompts = []
        for p in prompts:
            if str(p) and str(p).strip():
                clean_prompts.append(str(p).strip())
