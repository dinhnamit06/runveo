...
def _make_audio(
    ...
    *,
    ...
    voice_name = _voice_name(voice_key)
    ...
    if provider == "off" or str(voice_key or "").strip() == "None_NoVoice":
...
def create_tts_preview(
    ...
    *,
    ...
    cache_key = hashlib.md5(f"{clean_text}_{voice_key}_{provider}".encode("utf-8")).hexdigest()
    cached_path = out_root / f"preview_{cache_key}.mp3"
...
def _make_motion_clip(image_path: str, audio_path: str, out_path: str, duration: float, aspect_ratio: str) -> None:
...
def export_storytelling_video(
    ...
    *,
    ...
    for idx, item in enumerate(clean_items, start=1):
        log(f"🎙️ Tạo voice và motion ảnh storytelling {idx}/{len(clean_items)}...")
        ...
        _make_motion_clip(item["image_path"], audio_path, clip_path, duration, aspect_ratio)
