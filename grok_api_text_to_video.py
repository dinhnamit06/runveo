...
async def verify_grok_session(page) -> bool:
    """Verify if the user is logged into Grok.
    Returns:
    ...
    try:
        ...
        # Try to find common post-login elements or account menu
        # Grok usually has 'Account' or profile icons after login
        is_logged_in = await page.evaluate(
            """() => {
                const text = document.body.innerText || "";
                if (text.includes("Sign in") && !text.includes("Sign out")) return false;
                // Check for drawer or account button
                if (document.querySelector('a[href*="/settings"]')) return true;
...
async def auto_discover_statsig_headers(
    ...
    if "/login" in (page.url or "").lower():
        print("⚠️ Current page is login, skipping statsig discovery.")
        return {}
    ...
    try:
        try:
            # If not already on imagine/imagine related page, go there
            curr_url = page.url or ""
            if "grok.com/imagine" not in curr_url and "grok.com/" in curr_url:
        ...
        if "/login" in (page.url or "").lower():
            print("⚠️ Redirected to login page during discovery.")
            return {}
...
async def download_mp4(context, url: str, out_path: Path, timeout_ms: int, statsig: dict = None) -> bool:
    ...
    for attempt in range(1, max_attempts + 1):
      ...
      try:
        ...
        if resp.status == 404:
            print(f"⚠️ 404 Not Found: {target}")
            return False
        if resp.status != 200:
            print(f"⚠️ Status {resp.status} for {target}")
            continue
...
async def probe_video_url(context, url: str, timeout_ms: int = 10000, statsig: dict = None) -> str | None:
    """
    Lightweight check if a URL is a valid video asset. Returns the URL if valid, else None.
    """
    ...
    try:
        # Use simple GET to check headers
        resp = await context.request.get(target, timeout=timeout_ms, headers=headers)
        if resp.status == 200:
            ctype = (resp.headers.get("content-type") or "").lower()
