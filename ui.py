...
class AppConfig:
    ...
    def load(cls) -> "AppConfig":
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                cfg.idea_scene_count = int(
                    data.get("IDEA_SCENE_COUNT", data.get("idea_scene_count", cfg.idea_scene_count))
                    or cfg.idea_scene_count
                )
        ...
        try:
            cfg.idea_scene_count = max(1, min(100, int(cfg.idea_scene_count or 1)))
        except Exception:
    ...
    def save(self) -> None:
        os.makedirs(DATA_GENERAL_DIR, exist_ok=True)
        # Preserve existing account/token fields if present.
        existing: dict = {}
        try:
        ...
        account1_existing = existing.get("account1") if isinstance(existing.get("account1"), dict) else {}
        account1 = dict(account1_existing or {})
        email = str(self.veo3_user or self.USER or account1.get("email") or "")
        if email:
            account1["email"] = email
        # Save password as requested.
        pw = str(self.veo3_pass or self.PASS or account1.get("password") or "")
        if pw:
        ...
        grok_account_existing = existing.get("grok_account") if isinstance(existing.get("grok_account"), dict) else {}
        grok_account = dict(grok_account_existing or {})
        grok_email = str(self.grok_user or grok_account.get("email") or "")
        if grok_email:
            grok_account["email"] = grok_email
        grok_password = str(self.grok_pass or grok_account.get("password") or "")
        if grok_password:
            grok_account["password"] = grok_password
        grok_account_type = str(getattr(self, "grok_account_type", "SUPER") or "SUPER").strip().upper()
        ...
        data = {
            "MULTI_VIDEO": int(self.multi_video or 1),
            "OUTPUT_COUNT": int(self.output_count or 1),
            "CREATE_IMAGE_MODEL": str(self.create_image_model or "Imagen 4"),
            "WAIT_GEN_VIDEO": int(self.wait_gen_video or 15),
            "WAIT_GEN_IMAGE": int(self.wait_gen_image or 15),
            "CLEAR_DATA_IMAGE": int(self.CLEAR_DATA_IMAGE or 11),
            "RETRY_WITH_ERROR": int(self.retry_with_error or 3),
            "CLEAR_DATA": int(self.clear_data or 5),
            "CLEAR_DATA_WAIT": int(self.clear_data_wait or 4),
            "WAIT_RESEND_VIDEO": int(self.wait_resend_video or 10),
            ...
            "SEED_VALUE": int(self.seed_value or 9797),
        ...
        # Keep offscreen_chrome in the file, but do NOT change it on save.
        ...
        # Persist Gemini keys to text file (one key per line).
        try:
