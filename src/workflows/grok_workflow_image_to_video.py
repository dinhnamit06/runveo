...
from src.api.grok_api_image_to_video import (
    ImageToVideoConfig,
...
from src.api.grok_api_text_to_video import (
...
async def _run_jobs_async_ui(
    ...
    try:
        ...
        _safe_call(on_info, "Kiểm tra trạng thái đăng nhập")
        is_logged_in = await verify_grok_session(page)
        if not is_logged_in:
            _safe_call(on_info, "⚠️ Chưa đăng nhập GROK hoặc session hết hạn. Hãy đăng nhập lại trong tab Settings.")

        _safe_call(on_info, f"Tạo pool với tối đa {max_concurrency} tab để chạy song song...")
        pages = [page]
        needed_pages = min(max_concurrency, len(items)) - 1
        for _ in range(max(0, needed_pages)):
            try:
                if _stop_requested(): break
                p = await context.new_page()
                await p.goto("https://grok.com/", wait_until="domcontentloaded", timeout=30000)
                pages.append(p)
            except Exception:
                pass
        ...
        try:
            for p in pages:
                await p.expose_function("py_progress", js_progress)
        except Exception:
            pass
        try:
            for p in pages:
                await p.expose_function("py_log", js_log)
        except Exception:
            pass
        ...
        async def run_one_job(idx: int, job: dict[str, Any]) -> None:
            nonlocal headers
            if _stop_requested():
                _safe_call(on_status, idx, "Stop")
                return

            raw_paths = job.get("image_paths") or [job.get("image_path")]
            image_paths = [Path(str(p).strip()) for p in raw_paths if str(p).strip()]
            prompt = str(job.get("prompt") or "").strip()

            if not image_paths:
                _safe_call(on_status, idx, "Lỗi ảnh")
                _safe_call(on_info, f"[GROK-I2V {idx+1}] không có ảnh hợp lệ")
                return

            _safe_call(on_info, f"[GROK-I2V {idx+1}] bắt đầu job | images={len(image_paths)}")
            _safe_call(
                on_info,
            ...
            for img_p in image_paths:
                if not img_p.is_file():
                    _safe_call(on_status, idx, "Lỗi ảnh")
                    _safe_call(on_info, f"[GROK-I2V {idx+1}] ảnh không tồn tại: {img_p}")
                    return
            ...
            async with sem:
                ...
                assigned_page = pages[idx % len(pages)]

                try:
                    if assigned_page.is_closed():
                        _safe_call(on_status, idx, "Lỗi Browser")
                        _safe_call(on_info, f"[GROK-I2V {idx+1}] Lỗi: Browser hoặc Page đã bị đóng.")
                        return
                except Exception:
                    _safe_call(on_status, idx, "Lỗi Browser")
                    _safe_call(on_info, f"[GROK-I2V {idx+1}] Lỗi: Không thể kết nối tới Browser.")
                    return
                ...
                for i, img_p in enumerate(image_paths):
                    ...
                    for attempt in range(1, 4):
                        upload_result = await _await_with_stop(
                            api_upload_image_in_page(assigned_page, img_p, headers),
                            step_name=f"upload ảnh {i+1} (lần {attempt})",
                            idx=idx,
                        )
                        status = int(upload_result.get("status") or 0)
                        if status in {401, 403}:
                            _safe_call(on_info, f"[GROK-I2V {idx+1}] Lỗi auth {status}. Đang refresh headers...")
                            async with headers_lock:
                                headers = await auto_discover_statsig_headers(assigned_page, CACHE_PATH, PROFILE_NAME, force=True, persist=False)
                            continue
                        break

                    mid = str(upload_result.get("fileMetadataId") or "").strip()
                    uri = str(upload_result.get("fileUri") or "").strip()
                    if not mid:
                        _safe_call(on_status, idx, "Lỗi upload")
                        _safe_call(on_info, f"[GROK-I2V {idx+1}] upload lỗi ảnh {i+1}: {upload_result}")
                        return
                    
                    file_metadata_ids.append(mid)
                    file_uris.append(uri)

                    # Create post for each image
                    media_url = str(uri or "").strip()
                    ...
                    for attempt in range(1, 3):
                        create_result = await _await_with_stop(
                            ...
                            step_name=f"tạo post ảnh {i+1} (lần {attempt})",
                            idx=idx,
                        )
                        status = int(create_result.get("status") or 0)
                        if status in {401, 403}:
                            async with headers_lock:
                                headers = await auto_discover_statsig_headers(assigned_page, CACHE_PATH, PROFILE_NAME, force=True, persist=False)
                            continue
                    ...
                    pid = str(create_result.get("postId") or "").strip()
                    if pid:
                        parent_post_ids.append(pid)
                        # Optionally update URI if post creation returned a better one
                        murl = str(create_result.get("mediaUrl") or "").strip()
                        if murl:
                            file_uris[-1] = murl
                
                _safe_call(on_status, idx, "Đang Tạo video")
                _safe_call(on_info, f"[GROK-I2V {idx+1}] gửi request tạo video với {len(file_metadata_ids)} attachments...")
                ...
                primary_parent = parent_post_ids[0] if parent_post_ids else (file_metadata_ids[0] if file_metadata_ids else "")
                generated_id = str(video_result.get("generatedId") or video_id or primary_parent or "").strip()
                ...
                def _norm(u):
                    ...
                    return f"https://assets.grok.com/{u.lstrip('/')}"

                n_source = _norm(source_url)
                if n_source:
                    candidate_urls.append(n_source)
                ...
                n_video = _norm(video_url)
                if n_video:
                    candidate_urls.append(n_video)
                ...
                try:
                    if file_uris:
                        parts = [p for p in str(file_uris[0]).split("/") if p]
                ...
                if uid and generated_id:
                    candidate_urls.append(
                        f"https://assets.grok.com/users/{uid}/generated/{generated_id}/generated_video_hd.mp4?cache=1&dl=1"
                    )
                    candidate_urls.append(
                ...
                main_img_path = image_paths[0] if image_paths else Path("unknown.png")
                out_path = DOWNLOAD_DIR / _build_unique_video_name(idx + 1, prompt, str(main_img_path))
                _safe_call(on_status, idx, "Đang Tải video")
                _safe_call(on_info, f"[GROK-I2V {idx+1}] tải video về: {out_path.name}")
                # Parallel Probing Optimization
                _safe_call(on_info, f"[GROK-I2V {idx+1}] đang tìm link tải tối ưu trong {len(dedup_urls)} ứng viên...")
                
                working_url = None
                if dedup_urls:
                    probe_tasks = [probe_video_url(context, u, timeout_ms=8000, statsig=headers) for u in dedup_urls]
                    for comp in asyncio.as_completed(probe_tasks):
                        if _stop_requested(): break
                        found = await comp
                        if found:
                            working_url = found
                ...
                ok = False
                if working_url:
                    _safe_call(on_info, f"[GROK-I2V {idx+1}] đã tìm thấy link: {working_url}")
                    ok = await _await_with_stop(
                        download_mp4(context, working_url, out_path, timeout_ms=DOWNLOAD_TIMEOUT_MS, statsig=headers),
                        step_name="tải video",
                        idx=idx,
                    )
                else:
                    _safe_call(on_info, f"[GROK-I2V {idx+1}] không tìm thấy link qua probe, thử tải fallback link đầu tiên")
                    ok = await _await_with_stop(
                        download_mp4(context, dedup_urls[0], out_path, timeout_ms=DOWNLOAD_TIMEOUT_MS, statsig=headers),
                        step_name="tải video fallback",
                        idx=idx,
                    )
...
def run_image_to_video_jobs(
    ...
    for raw in items or []:
        if not isinstance(raw, dict):
            continue
        image_link = str(raw.get("image_link") or raw.get("image_path") or "").strip()
        if not image_link:
            continue
        
        # Split by | to support multiple images per prompt
        paths = [p.strip() for p in image_link.split("|") if p.strip()]
        if not paths:
            continue
