...
from src.core.settings_manager import SettingsManager, DATA_GENERAL_DIR, WORKFLOWS_DIR
from src.workflows.A_workflow_get_token import TokenCollector
from src.api import API_text_to_video as t2v_api
from src.api.chrome_process_manager import ChromeProcessManager
from src.workflows.workflow_run_control import get_running_video_count, get_max_in_flight
...
class TextToVideoWorkflow(QThread):
	...
	def _run_with_new_loop(self):
		...
		try:
			...
			try:
				...
				from src.api.chrome import resolve_profile_dir
	...
	async def _run_workflow(self):
		...
		enable_concurrent = bool(config.get("ENABLE_CONCURRENT", True))
		multi_val = self._resolve_int_config(config, "MULTI_VIDEO", 1) if enable_concurrent else 1
		max_in_flight = self._resolve_worker_max_in_flight(multi_val)
		...
		try:
			...
			token_request_lock = asyncio.Lock()
			
			async def run_one_job(idx_prompt, prompt):
				nonlocal token_request_count, prompt_retry_counts
				try:
					prompt_id = prompt["id"]
					...
					while True:
						...
						sleep_time = wait_gen_video
						if self._count_in_progress() < max_in_flight:
							sleep_time = min(3, wait_gen_video)
						
						if not await self._sleep_with_stop(sleep_time):
							break
						break
				
				except Exception as e:
					self._log(f"❌ Lỗi xử lý prompt {prompt.get('id', 'UNKNOWN')}: {e}")
					import traceback
					self._log(f"Traceback: {traceback.format_exc()}")
			
			async with collector:
				# Mảng để lưu các task gửi request
				for i in range(0, len(prompts), max_in_flight):
					if self.STOP:
					...
					batch = prompts[i:i + max_in_flight]
					multi_tasks = []
					for idx_prompt_in_batch, prompt in enumerate(batch):
						idx_prompt = i + idx_prompt_in_batch
						multi_tasks.append(
							asyncio.create_task(run_one_job(idx_prompt, prompt))
						)
					await asyncio.gather(*multi_tasks)
					if self.STOP:
						break
