...
async def _run_jobs_async_ui(
  ...
  try:
    ...
    pages = [page]
    needed_pages = min(max_concurrency, len(prompts)) - 1
    ...
    async def _upscale_again(p_eval, video_id: str, statsig_headers: dict, job_index: int) -> str | None:
      if _stop_requested():
        return None
      vid = str(video_id or "").strip()
      if not vid:
      ...
      async with sem:
        ...
        try:
          if assigned_page.is_closed():
            ...
            _safe_call(on_info, f"[GROK-T2V {idx+1}] Lỗi: Browser hoặc Page đã bị đóng.")
            return
        except Exception:
          _safe_call(on_status, idx, "Lỗi Browser")
          _safe_call(on_info, f"[GROK-T2V {idx+1}] Lỗi: Không thể kết nối tới Browser.")
          return
        ...
        job = results[0] if isinstance(results, list) and results else {}
        status_code = int(job.get("convoStatus") or job.get("createStatus") or 0)
        
        if status_code in {401, 403}:
          async with headers_lock:
            _safe_call(on_info, f"Refresh headers (job {idx + 1})")
            headers = await _await_with_stop(
              auto_discover_statsig_headers(assigned_page, CACHE_PATH, PROFILE_NAME, force=True, persist=False),
        ...
        # Build candidate URLs for robust downloading
        ...
        n_upscale = _norm(upscale_url)
        if n_upscale:
          candidate_urls.append(n_upscale)
        n_media = _norm(media_url)
        if n_media:
          candidate_urls.append(n_media)
        ...
        for u in candidate_urls:
            ...
            if u_clean and u_clean not in seen_urls:
                dedup_urls.append(u_clean)
                seen_urls.add(u_clean)

        if not dedup_urls:
            _safe_call(on_status, idx, "Lỗi URL")
            _safe_call(on_info, f"[GROK-T2V {idx+1}] không có URL tải nào khả dụng")
            return
        ...
        _safe_call(on_info, f"[GROK-T2V {idx+1}] đang tìm link tải tối ưu trong {len(dedup_urls)} ứng viên...")
        
        working_url = None
        if dedup_urls:
            probe_tasks = [probe_video_url(context, u, timeout_ms=8000, statsig=local_headers) for u in dedup_urls]
            for comp in asyncio.as_completed(probe_tasks):
                if _stop_requested(): break
                found = await comp
                if found:
                    working_url = found
                    break # Take the first one that works
        
        ok = False
        if working_url:
            _safe_call(on_info, f"[GROK-T2V {idx+1}] đã tìm thấy link: {working_url}")
            ok = await _await_with_stop(
                download_mp4(context, working_url, out_path, timeout_ms=DOWNLOAD_TIMEOUT_MS, statsig=local_headers),
                step_name="tải video",
                idx=idx,
            )
        else:
            _safe_call(on_info, f"[GROK-T2V {idx+1}] không tìm thấy link nào qua probe, thử tải tuần tự link đầu tiên")
            ok = await _await_with_stop(
                download_mp4(context, dedup_urls[0], out_path, timeout_ms=DOWNLOAD_TIMEOUT_MS, statsig=local_headers),
        ...
        if ok:
            ...
            _safe_call(on_info, f"[GROK-T2V {idx+1}] tải thất bại với tất cả candidate, thử upscale lại")

            if generated_id:
                _safe_call(on_status, idx, "Upscale lại")
                fresh_hd = await _upscale_again(assigned_page, generated_id, local_headers, job_index=idx)
                if fresh_hd:
                    _safe_call(on_status, idx, "Tải lại HD")
                    ok2 = await _await_with_stop(
                        download_mp4(context, fresh_hd.strip(), out_path, timeout_ms=DOWNLOAD_TIMEOUT_MS, statsig=local_headers),
            ...
            _safe_call(on_info, f"[GROK-T2V {idx+1}] tải thất bại hoàn toàn")
