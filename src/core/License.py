...
from src.core.branding_config import (
    APP_VERSION,
...
# ===== CONFIG =====
URL = "https://script.google.com/macros/s/AKfycbxeF74Zk4WbosH6XTfQBzWxJUyv-zeeYhgWWYNXEHg_WFLWBLfv4kwkNWwqpdH09k7W2A/exec"
...
def _check_license(license_key: str):
    ...
    # Ký request để chống giả mạo request gửi lên server
    msg = canonical_request(license_key, machine_id, ts, nonce)
    sig = sign_hmac_hex(LICENSE_SECRET, msg)
    ...
    try:
        ...
        if resp.status_code != 200:
            return resp.status_code, {"error": f"Server connection failed ({resp.status_code})"}, elapsed
            
        try:
            data = resp.json()
        except Exception:
            return 200, {"error": "Invalid server response format"}, elapsed

        # 1. Kiểm tra sự tồn tại của chữ ký server
        server_sig = data.get("server_sig")
        if not server_sig:
             return 200, {"ok": False, "reason": "Missing server signature"}, elapsed
             
        # 2. Xác minh chữ ký server (Chống giả cập server/Crack)
        ok_val = data.get("ok", False)
        ok_str = "true" if ok_val else "false"
        
        # ÉP BUỘC: Luôn dùng key và nonce gốc để đảm bảo SIGNATURE luôn có giá trị
        key_val = str(license_key)
        nonce_val = str(nonce)
        
        # Chỉ ký các trường cốt lõi để đảm bảo độ tin cậy tuyệt đối
        resp_msg = f"ok={ok_str}&key={key_val}&nonce={nonce_val}"
        expected_server_sig = sign_hmac_hex(LICENSE_SECRET, resp_msg)
        
        if server_sig != expected_server_sig:
            messagebox.showerror("Security Error", "Security Mismatch (Invalid server signature).\n\nVui lòng không can thiệp vào mã nguồn hoặc liên hệ Admin để được hỗ trợ.")
            return 200, {"ok": False, "reason": "Security Mismatch"}, elapsed
        
        return 200, data, elapsed

    except Exception as e:
        return 0, {"error": "Connection error: " + str(e)}, 0
...
def _run_app():
    if getattr(sys, "frozen", False):
        try:
            from src.core import main as main_module
