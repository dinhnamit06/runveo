...
from src.ui.tab_text_to_video import PromptEditor
from src.core.settings_manager import get_icon_path
...
class _SingleImageTab(QWidget):
    def __init__(self, parent: QWidget | None = None):
        ...
        self._thumb_indices: dict[int, int] = {} # Store current visible index per row
    ...
    def _duplicate_row(self, idx: int) -> None:
        i = int(idx)
        max_len = max(len(self._images), len(self._prompt_overrides))
        if i < 0 or i >= max_len:
            return
        ...
        self._ensure_rows(max_len)
        src_image = self._images[i]
        
        # Ask for count
        count, ok = QInputDialog.getInt(
            self, "Nhân bản dòng", f"Số lượng bản sao cho dòng {i+1}:", 1, 1, 100, 1
        )
        ...
        for _ in range(count):
            self._images.append(src_image)
            self._prompt_overrides.append("") # Empty prompt as requested
    ...
    def _build_image_cell(self, idx: int, path: str) -> tuple[QWidget, int]:
        ...
        if paths:
            curr_idx = self._thumb_indices.get(idx, 0)
            if curr_idx >= len(paths):
                curr_idx = 0
            
            primary_path = paths[curr_idx]
            try:
                pix = QPixmap(primary_path) if primary_path else QPixmap()
                if not pix.isNull():
                    pix = pix.scaled(
                        206,
                        124,
        ...
        if len(paths) > 1:
            curr_idx = self._thumb_indices.get(idx, 0)
            txt = f"[{curr_idx + 1}/{len(paths)}] " + os.path.basename(paths[curr_idx])
        else:
        ...
        if len(paths) > 1:
            ...
            row_slider.setContentsMargins(0, 0, 0, 0)
            row_slider.setSpacing(8)
            
            def _change_slide(delta, row_idx=idx, total=len(paths)):
                curr = self._thumb_indices.get(row_idx, 0)
                new_idx = (curr + delta) % total
                self._thumb_indices[row_idx] = new_idx
                self._sync_table()

            btn_l = QPushButton("◀")
            btn_r = QPushButton("▶")
            for b_sl in (btn_l, btn_r):
                b_sl.setFixedSize(22, 22)
                b_sl.setStyleSheet(
                    "QPushButton{background:#f1f5f9; border:1px solid #cbd5e1; border-radius:11px; color:#475569; font-size:10px; padding:0px;}"
                    "QPushButton:hover{background:#e2e8f0;}"
                )
            btn_l.clicked.connect(lambda: _change_slide(-1))
            btn_r.clicked.connect(lambda: _change_slide(1))
            
            row_slider.addStretch(1)
            row_slider.addWidget(btn_l)
            row_slider.addWidget(thumb)
            row_slider.addWidget(btn_r)
            row_slider.addStretch(1)
            l.addLayout(row_slider)
        else:
        ...
        btn_dup = QPushButton("++")
        btn_dup.setFixedSize(22, 22)
        btn_dup.setToolTip("Nhân bản dòng này")
        btn_dup.setStyleSheet(
            "background:#fef3c7; border:1px solid #fde68a; border-radius:11px;"
            "color:#92400e; font-weight:800; font-size:11px; padding:0px;"
        )
        btn_dup.clicked.connect(lambda _=False, i=idx: self._duplicate_row(i))
        ...
        b.addWidget(btn_dup, 0, Qt.AlignmentFlag.AlignHCenter)
        b.addStretch(1)
    ...
    def _pick_single(self, idx: int) -> None:
        ...
        files, _ = QFileDialog.getOpenFileNames(
            self,
            f"Chọn một hoặc nhiều ảnh (dòng {i+1})",
            "",
            "Images (*.png *.jpg *.jpeg *.webp *.bmp);;All Files (*.*)",
        )
        if not files:
            return
        combined = "|".join([str(f) for f in files])
        self._ensure_rows(i + 1)
        self._images[i] = combined
        self._sync_table()
    ...
    def pick_images(self) -> None:
        ...
        # Handle multiple images choice
        if len(picked) > 1:
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Icon.Question)
            msg.setWindowTitle("Tùy chọn nhập ảnh")
            msg.setText(f"Bạn đã chọn {len(picked)} ảnh. Bạn muốn nhập như thế nào?")
            
            btn_group = msg.addButton("Tất cả vào 1 dòng (Tham chiếu nhiều ảnh)", QMessageBox.ButtonRole.ActionRole)
            btn_bulk = msg.addButton("Mỗi ảnh 1 dòng (Tạo hàng loạt)", QMessageBox.ButtonRole.ActionRole)
            btn_cancel = msg.addButton("Hủy", QMessageBox.ButtonRole.RejectRole)
            
            msg.setDefaultButton(btn_group)
            msg.exec()
