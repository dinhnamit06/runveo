...
from src.ui.tab_text_to_video import TextToVideoTab
from src.ui.tab_image_to_video import ImageToVideoTab
from src.ui.tab_character_sync import CharacterSyncTab
from src.ui.tab_create_image import CreateImageTab
from src.ui.tab_create_image import CreateImageFromPromptTab
from src.ui.tab_idea_to_video import IdeaToVideoTab
from src.ui.tab_copy_video import CopyVideoTab
from src.ui.popup_theme import install_messagebox_theme
from src.ui.tab_settings import SettingsTab
from src.ui.tab_grok_settings import GrokSettingsTab
from src.core.branding_config import WINDOW_TITLE
from src.core.settings_manager import BASE_DIR, get_icon_path
from src.utils.voice_profiles import get_voice_locale
...
def icon(name: str) -> QIcon:
    ...
    ic = QIcon(path) if os.path.isfile(path) else QIcon()
    _icon_cache[name] = ic
    return ic
...
def app_logo_icon() -> QIcon:
    ...
    try:
        for filename in os.listdir(icon_dir):
            if not filename.lower().endswith((".ico", ".png", ".jpg", ".jpeg", ".bmp", ".svg")):
                continue
...
class AppConfig:
    ...
    idea_storytelling_motion: bool = True
    ...
    grok_user: str = ""
    grok_pass: str = ""
    grok_account_type: str = "SUPER"  # SUPER|NORMAL
    ...
    def load(cls) -> "AppConfig":
        ...
        try:
            ...
            if not os.path.isfile(load_path) and os.path.isfile(OLD_CONFIG_PATH):
                load_path = OLD_CONFIG_PATH

            with open(load_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            if isinstance(data, dict):
                # New config.json schema (uppercase keys)
                cfg.enable_concurrent = bool(data.get("ENABLE_CONCURRENT", cfg.enable_concurrent))
                cfg.multi_video = int(data.get("MULTI_VIDEO", cfg.multi_video) or cfg.multi_video)
                cfg.tiktok_session_id = str(data.get("TIKTOK_SESSION_ID", cfg.tiktok_session_id) or "")
                cfg.output_count = int(data.get("OUTPUT_COUNT", cfg.output_count) or cfg.output_count)
                cfg.create_image_model = str(
                    data.get("CREATE_IMAGE_MODEL", data.get("create_image_model", cfg.create_image_model))
                    or cfg.create_image_model
                )
                cfg.wait_gen_video = int(data.get("WAIT_GEN_VIDEO", cfg.wait_gen_video) or cfg.wait_gen_video)
                cfg.wait_gen_image = int(data.get("WAIT_GEN_IMAGE", cfg.wait_gen_image) or cfg.wait_gen_image)
                cfg.CLEAR_DATA_IMAGE = int(
                    data.get("CLEAR_DATA_IMAGE", cfg.CLEAR_DATA_IMAGE) or cfg.CLEAR_DATA_IMAGE
                )
                cfg.retry_with_error = int(data.get("RETRY_WITH_ERROR", cfg.retry_with_error) or cfg.retry_with_error)
                cfg.clear_data = int(data.get("CLEAR_DATA", cfg.clear_data) or cfg.clear_data)
                cfg.clear_data_wait = int(data.get("CLEAR_DATA_WAIT", cfg.clear_data_wait) or cfg.clear_data_wait)
                cfg.wait_resend_video = int(data.get("WAIT_RESEND_VIDEO", cfg.wait_resend_video) or cfg.wait_resend_video)
                cfg.download_mode = str(data.get("DOWNLOAD_MODE", cfg.download_mode) or cfg.download_mode)

                cfg.token_option = cls._config_token_option_from_json(data.get("TOKEN_OPTION"))
                cfg.seed_mode = str(data.get("SEED_MODE", cfg.seed_mode) or cfg.seed_mode)
                cfg.seed_value = int(data.get("SEED_VALUE", cfg.seed_value) or cfg.seed_value)

                cfg.video_aspect_ratio = str(
                    data.get("VIDEO_ASPECT_RATIO", data.get("video_aspect_ratio", cfg.video_aspect_ratio))
                    or cfg.video_aspect_ratio
                )
                cfg.veo_model = str(data.get("VEO_MODEL", data.get("veo_model", cfg.veo_model)) or cfg.veo_model)
                cfg.video_output_dir = str(
                    data.get("VIDEO_OUTPUT_DIR", data.get("video_output_dir", cfg.video_output_dir))
                    or cfg.video_output_dir
                )
                raw_scene_count = data.get("IDEA_SCENE_COUNT", data.get("idea_scene_count", cfg.idea_scene_count))
                raw_scene_text = str(raw_scene_count or "").strip().casefold()
                if raw_scene_text in {"auto", "tu dong", "tự động"}:
                    cfg.idea_scene_count = 0
                else:
                    cfg.idea_scene_count = int(raw_scene_count)
                cfg.idea_style = str(data.get("IDEA_STYLE", data.get("idea_style", cfg.idea_style)) or cfg.idea_style)
                cfg.idea_dialogue_language = str(
                    data.get(
                        "IDEA_DIALOGUE_LANGUAGE",
                        data.get("idea_dialogue_language", cfg.idea_dialogue_language),
                    )
                ...
                cfg.idea_storytelling_motion = bool(data.get("IDEA_STORYTELLING_MOTION", cfg.idea_storytelling_motion))
                ...
                # Gemini keys: prefer text file; fallback to legacy JSON field if present.
                try:
                ...
                account1 = data.get("account1") if isinstance(data.get("account1"), dict) else {}
                if account1:
                    cfg.veo3_user = str(account1.get("email") or cfg.veo3_user or "")
                    cfg.veo3_pass = str(account1.get("password") or cfg.veo3_pass or "")

                grok_account = data.get("grok_account") if isinstance(data.get("grok_account"), dict) else {}
                if grok_account:
                    cfg.grok_user = str(grok_account.get("email") or cfg.grok_user or "")
                    cfg.grok_pass = str(grok_account.get("password") or cfg.grok_pass or "")
                    raw_grok_type = str(
                        grok_account.get("type_account")
                        or grok_account.get("TYPE_ACCOUNT")
                        or data.get("GROK_ACCOUNT_TYPE")
                        or cfg.grok_account_type
                        or "SUPER"
                    ).strip().upper()
                    if raw_grok_type == "ULTRA":
                        raw_grok_type = "SUPER"
                    cfg.grok_account_type = "NORMAL" if raw_grok_type == "NORMAL" else "SUPER"

                cfg.grok_video_length_seconds = int(
                    data.get("GROK_VIDEO_LENGTH_SECONDS", data.get("grok_video_length_seconds", cfg.grok_video_length_seconds))
                    or cfg.grok_video_length_seconds
                )
                cfg.grok_video_resolution = str(
        ...
        # Fixed defaults per existing behavior
        cfg.video_resolution = "480p"
        cfg.auto_upscale = True
        cfg.token_option = cls._config_token_option_from_json(cfg.token_option)
        if not str(cfg.video_output_dir or "").strip():
            cfg.video_output_dir = DEFAULT_DOWNLOAD_DIR
        cfg.idea_scene_count = cls._normalize_scene_count_value(cfg.idea_scene_count, default=1)
        if str(cfg.video_aspect_ratio or "") not in {"9:16", "16:9"}:
            cfg.video_aspect_ratio = "9:16"
        if str(cfg.veo_model or "") not in VEO_MODEL_OPTIONS:
        ...
        if str(cfg.idea_tts_provider or "") not in {"auto", "edge", "sapi", "tiktok", "off"}:
        ...
        raw_grok_type = str(getattr(cfg, "grok_account_type", "SUPER") or "SUPER").strip().upper()
        if raw_grok_type == "ULTRA":
            raw_grok_type = "SUPER"
        cfg.grok_account_type = "NORMAL" if raw_grok_type == "NORMAL" else "SUPER"
        return cfg
    ...
    @staticmethod
    def _normalize_scene_count_value(value, default: int = 1) -> int:
        raw = str(value or "").strip()
        if raw.casefold() in {"auto", "tu dong", "tự động"}:
            return 0
        try:
            count = int(value)
        except Exception:
    ...
    def save(self) -> None:
        ...
        account1 = dict(existing.get("account1")) if isinstance(existing.get("account1"), dict) else {}
        if str(self.veo3_user or "").strip():
            account1["email"] = str(self.veo3_user or "").strip()
        if str(self.veo3_pass or ""):
            account1["password"] = str(self.veo3_pass or "")

        grok_account = dict(existing.get("grok_account")) if isinstance(existing.get("grok_account"), dict) else {}
        grok_account_type = str(self.grok_account_type or "SUPER").strip().upper()
        if grok_account_type == "ULTRA":
            grok_account_type = "SUPER"
        grok_account_type = "NORMAL" if grok_account_type == "NORMAL" else "SUPER"
        ...
        data = {
            "MULTI_VIDEO": _as_int(self.multi_video, 3),
            "OUTPUT_COUNT": _as_int(self.output_count, 1),
            "CREATE_IMAGE_MODEL": str(self.create_image_model or "Imagen 4"),
            "WAIT_GEN_VIDEO": _as_int(self.wait_gen_video, 15),
            "WAIT_GEN_IMAGE": _as_int(self.wait_gen_image, 15),
            "CLEAR_DATA_IMAGE": _as_int(self.CLEAR_DATA_IMAGE, 11),
            "RETRY_WITH_ERROR": _as_int(self.retry_with_error, 3),
            "CLEAR_DATA": _as_int(self.clear_data, 5),
            "CLEAR_DATA_WAIT": _as_int(self.clear_data_wait, 4),
            "WAIT_RESEND_VIDEO": _as_int(self.wait_resend_video, 10),
            "DOWNLOAD_MODE": str(self.download_mode or "720"),
            "TOKEN_OPTION": self._config_token_option_for_json(),
            "SEED_MODE": str(self.seed_mode or "Random"),
            "SEED_VALUE": _as_int(self.seed_value, 9797),
            "VIDEO_ASPECT_RATIO": str(self.video_aspect_ratio or "9:16"),
            "VEO_MODEL": str(self.veo_model or VEO_MODEL_FAST),
            "VIDEO_OUTPUT_DIR": str(self.video_output_dir or DEFAULT_DOWNLOAD_DIR),
            "IDEA_SCENE_COUNT": self._normalize_scene_count_value(self.idea_scene_count, default=1),
            ...
            "IDEA_STORYTELLING_MOTION": bool(self.idea_storytelling_motion),
            ...
            "COPY_VIDEO_STRENGTH": max(50, min(100, _as_int(self.copy_video_strength, 100))),
            "COPY_VIDEO_USER_IDEA": str(self.copy_video_user_idea or ""),
            "GROK_VIDEO_LENGTH_SECONDS": 10 if _as_int(self.grok_video_length_seconds, 6) == 10 else 6,
...
def apply_style(app: QApplication) -> None:
    # Soft blue QSS with stronger readability.
    qss = """
    QWidget {
...
class MainWindow(QMainWindow):
    def __init__(self, config: AppConfig):
        super().__init__()
        self._cfg = config
        self.setWindowTitle(str(WINDOW_TITLE or "VEO TOOL"))
        ...
        self.tab_create_image = CreateImageTab(
            config=self._cfg,
        ...
        try:
            self.main_tabs.currentChanged.connect(lambda _=0: self._on_main_tab_changed())
        except Exception:
            pass
        try:
            self.tabs.currentChanged.connect(lambda _=0: self._update_start_button_for_tab())
        except Exception:
            pass
        try:
            self.grok_tabs.currentChanged.connect(lambda _=0: self._update_start_button_for_tab())
        except Exception:
            pass
        try:
            self.tab_create_image.tabs.currentChanged.connect(lambda _=0: self._update_start_button_for_tab())
        except Exception:
            pass
        try:
            self.tab_image.sub_tabs.currentChanged.connect(lambda _=0: self._update_start_button_for_tab())
        except Exception:
            pass
        ...
        idx = self.combo_aspect.findData(str(config.video_aspect_ratio or "9:16"))
        ...
        model_idx = self.combo_veo_model.findData(str(config.veo_model or VEO_MODEL_FAST))
        ...
        self.out_dir.setText(str(config.video_output_dir or DEFAULT_DOWNLOAD_DIR))
        ...
        # Increase left panel width by ~20% compared to the old 2:3 feel.
        # Old left share ~40%; new share ~48%.
    ...
    def _active_platform_prefix(self) -> str:
        try:
            if self.main_tabs.currentWidget() is self.main_tabs.widget(1):
                return "GROK"
        except Exception:
            pass
        return "VEO3"
    ...
    def _update_start_button_for_tab(self) -> None:
        ...
        busy = bool(self.status.isRunning() or self._queue_worker.is_busy())
        if busy:
        ...
        if cur is self.tab_text or cur is self.tab_grok_text:
            ...
            create_mode = "prompt"
            try:
                create_mode = str(self.tab_create_image.current_mode() or "prompt")
            except Exception:
                create_mode = "prompt"
            ...
            if cur is self.tab_image:
                mode = "single"
                try:
                    mode = str(self.tab_image.current_mode() or "single")
                except Exception:
                    mode = "single"
                if mode == "start_end":
                ...
                self.btn_start.setText(f"{platform_prefix} - Tạo Video Từ Ảnh")
            self.btn_start.setObjectName("Accent")
        elif cur is self.tab_idea or cur is getattr(self, "tab_grok_idea", None):
    ...
    def _get_type_account(self) -> str:
        try:
            ...
            if isinstance(data, dict):
                ...
                if raw in {"NORMAL", "PRO", "ULTRA"}:
                    return raw
        except Exception:
            pass

        raw_cfg = str(getattr(self._cfg, "TYPE_ACCOUNT", "") or "").strip().upper()
        if raw_cfg in {"NORMAL", "PRO", "ULTRA"}:
            return raw_cfg
        return "ULTRA"

    def _ensure_veo_model_allowed(self, show_message: bool = False) -> bool:
        selected = str(self.combo_veo_model.currentData() or VEO_MODEL_FAST)
        ...
        if show_message:
            QMessageBox.warning(
                self,
                "Model không hỗ trợ",
                "Model Fast 2.0 chỉ hỗ trợ tài khoản ULTRA.",
            )
        return False

    def _open_output_folder(self) -> None:
        out = self.out_dir.text().strip() or str(self._cfg.video_output_dir or DEFAULT_DOWNLOAD_DIR)
        try:
    ...
    def _confirm_add_to_queue(self, workflow_title: str, prompt_count: int) -> bool:
        count = max(0, int(prompt_count or 0))
        return (
            QMessageBox.question(
                self,
                "Xác nhận thêm hàng chờ",
                f"Bạn có muốn thêm {count} prompt {workflow_title} vào hàng chờ không?",
    ...
    def _notify_add_to_queue_success(self, workflow_title: str, prompt_count: int) -> None:
        ...
        QMessageBox.information(
            self,
            "Thêm hàng chờ",
            f"Đã thêm thành công {count} prompt {workflow_title} vào hàng chờ.",
        )

    def _start_queue_job(self, item: WorkflowQueueItem) -> bool:
        return bool(self.status.start_queued_job(item.mode_key, item.rows))
    ...
    def _enqueue_payload(self, payload: dict | None) -> None:
        ...
        label = str(payload.get("label") or mode_key or "workflow")
        pending = self._queue_worker.enqueue(WorkflowQueueItem(mode_key=mode_key, rows=rows, label=label))
    ...
    def _enqueue_jobs(self, jobs: list) -> None:
        for payload in list(jobs or []):
            if isinstance(payload, dict):
                self._enqueue_payload(payload)

    def _on_queue_tick(self) -> None:
        if self._queue_worker.is_stopping():
            return
        self._queue_worker.ensure_started()

    def _on_stop_all(self) -> None:
        is_busy = bool(self.status.isRunning() or self._queue_worker.is_busy() or self._queue_worker.is_stopping())
        if not is_busy:
        ...
        confirm = QMessageBox.question(
            self,
            "Xác nhận dừng",
            "Bạn có chắc muốn dừng workflow đang chạy và xóa toàn bộ hàng chờ?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def _on_start_stop(self) -> None:
        ...
        if flow_name == "settings":
            QMessageBox.information(
                self,
                "Cài đặt",
                "Tab Cài đặt không chạy workflow.",
            )
            return
        ...
        if flow_name == "grok_create_image_prompt":
            ...
            if not prompts:
                ...
                return
            items = [{"id": str(i + 1), "description": str(p)} for i, p in enumerate(prompts) if str(p).strip()]
            payload = self.status.enqueue_generate_image_from_prompts(items)
            self._enqueue_payload(payload)
            return
        ...
        if flow_name in ("idea_to_video", "grok_idea_to_video"):
            if self.status.isRunning() or self._queue_worker.is_busy():
        ...
        if flow_name == "image_to_video":
            ...
            if mode == "start_end":
                invalid = [
                    ...
                    if not str(item.get("start_image_link") or "").strip()
                    or not str(item.get("end_image_link") or "").strip()
                ]
                if invalid:
                    ...
                    QMessageBox.warning(
                        self,
                        "Thiếu ảnh",
                        f"Mode Ảnh Đầu - Ảnh Cuối yêu cầu đủ 2 ảnh cho mỗi dòng.\nDòng thiếu: {preview}{more}",
                    )
                    return
            else:
                invalid = [
                ...
                if invalid:
                    ...
                    QMessageBox.warning(
                        self,
                        "Thiếu ảnh",
                        f"Mode Tạo Video Từ Ảnh yêu cầu ảnh đầu vào cho mỗi dòng.\nDòng thiếu: {preview}{more}",
                    )
                    return
        ...
        if flow_name == "character_sync" or flow_name == "grok_character_sync":
            ...
            missing_names = [idx + 1 for idx, ch in enumerate(characters) if not str(ch.get("name") or "").strip()]
            if missing_names:
                ...
                QMessageBox.warning(
                    self,
                    "Thiếu tên nhân vật",
                    f"Các ảnh chưa đặt tên nhân vật: {preview}{more}",
                )
                return
        ...
        if flow_name != "text_to_video" and flow_name != "grok_text_to_video":
            QMessageBox.information(
                self,
                "Chưa hỗ trợ",
                f"Tab hiện tại là '{flow_name}'. Hiện chỉ hỗ trợ chạy luồng Text to Video.",
            )
            return
        ...
        text_tab = self.tab_grok_text if flow_name == "grok_text_to_video" else self.tab_text
        ...
        add_queue_mode = self._is_add_to_queue_mode()
        if add_queue_mode and not self._confirm_add_to_queue("tạo video từ văn bản", len(prompts)):
            return
