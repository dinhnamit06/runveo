...
class _AutoLoginWorker(QObject):
    ...
    def run(self) -> None:
        try:
            from src.ui.login import auto_login_veo3
...
class SettingsTab(QWidget):
    ...
    def __init__(self, config, parent: QWidget | None = None):
        ...
        self.enable_concurrent = QCheckBox("Bật chạy song song đa luồng")
        self.enable_concurrent.setChecked(bool(getattr(config, "enable_concurrent", True)))
        form.addRow("", self.enable_concurrent)
    ...
    def _save(self) -> None:
        ...
        setattr(self._cfg, "enable_concurrent", bool(self.enable_concurrent.isChecked()))
        try:
    ...
    def _profile_dir(self) -> Path:
        ...
        try:
            ...
            from src.core.settings_manager import BASE_DIR
            root = Path(BASE_DIR)
    ...
    def _open_profile(self) -> None:
        try:
            ...
            try:
                from src.api.chrome import open_profile_chrome
                from src.core.settings_manager import SettingsManager
    ...
    def _show_profile_popup(self, profile_dir: str, host: str, port: int) -> None:
        if self._profile_popup is None:
            ...
            status.setStyleSheet("color:#1f2d48; font-size:12px; background:#eef5ff; border:1px solid #c8d7f2; border-radius:8px; padding:8px;")
    ...
    def _close_profile_chrome(self) -> None:
        try:
            ...
            if not profile_dir:
                return
            from src.api.chrome import kill_profile_chrome
    ...
    def _show_auto_login_popup(self, profile_name: str) -> None:
        if self._auto_login_popup is None:
            ...
            log_view.setStyleSheet("background:#ffffff; color:#1f2d48; border:1px solid #c8d7f2;")
