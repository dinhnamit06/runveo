...
from src.api.grok_chrome_manager import kill_profile_chrome, open_profile_chrome, resolve_profile_dir
from src.core.settings_manager import DATA_GENERAL_DIR
from src.utils.voice_profiles import VOICE_JSON, VOICE_OPTIONS
...
class GrokSettingsTab(QWidget):
    def __init__(self, config, parent: QWidget | None = None):
        ...
        current_type = str(getattr(config, "grok_account_type", "SUPER") or "SUPER").strip().upper()
        if current_type == "ULTRA":
            current_type = "SUPER"
        idx_type = self.grok_account_type.findData("NORMAL" if current_type == "NORMAL" else "SUPER")
        ...
        cur_len = int(getattr(config, "grok_video_length_seconds", 6) or 6)
        ...
        cur_res = str(getattr(config, "grok_video_resolution", "480p") or "480p")
        ...
        note = QLabel(
            "<b>Hướng Dẫn:</b><br/>"
            "Bước 1: Bấm nút <b>Mở Chrome để đăng nhập GROK</b>, sau đó nhập tài khoản và mật khẩu để đăng nhập GROK trực tiếp trên web.<br/>"
            "Bước 2: Chọn <b>Loại Tài khoản</b> đã đăng nhập. Tài khoản thường chọn <b>NORMAL</b>, tài khoản SUPER chọn <b>SUPER</b> "
            "(chọn sai sẽ không chạy được).<br/>"
            "Bước 3: Cấu hình thời gian video <b>6s hoặc 10s</b> (tài khoản <b>SUPER</b> mới chọn được 10s).<br/>"
            "Bước 4: Chọn chất lượng video <b>480 hoặc 720</b> (tool đã tự upscale lên 720 mức cao nhất của GROK rồi)."
        )
        note.setWordWrap(True)
        note.setStyleSheet(
            "color:#1f2d48; font-size:12px; line-height:1.45; "
    ...
    def _save(self) -> None:
        account_type = str(self.grok_account_type.currentData() or "SUPER").strip().upper()
        if account_type == "ULTRA":
            account_type = "SUPER"
        account_type = "NORMAL" if account_type == "NORMAL" else "SUPER"
        ...
        try:
            ...
            multi_video_value = 5
        if multi_video_value < 1:
            multi_video_value = 1
        if multi_video_value > 20:
    ...
    def _is_normal_account(self) -> bool:
        val = str(self.grok_account_type.currentData() or "SUPER").strip().upper()
        if val == "ULTRA":
            val = "SUPER"
        return val == "NORMAL"
    ...
    def _enforce_current_constraints(self) -> None:
        if not self._is_normal_account():
            return
    ...
    def _load_grok_runtime_config(self) -> dict:
        try:
            if GROK_CONFIG_PATH.is_file():
                raw = json.loads(GROK_CONFIG_PATH.read_text(encoding="utf-8"))
    ...
    def _open_grok_profile(self) -> None:
        ...
        try:
            ...
            self._last_profile_dir = str(opened.get("profile_dir") or self._profile_dir())
            self._show_profile_popup(
                profile_dir=self._last_profile_dir,
                host=str(opened.get("host") or "127.0.0.1"),
    ...
    def _show_profile_popup(self, profile_dir: str, host: str, port: int) -> None:
        if self._profile_popup is None:
            ...
            info.setStyleSheet("color:#1f2d48; font-size:12px;")
            layout.addWidget(info)

            status = QLabel("")
            status.setWordWrap(True)
            status.setStyleSheet(
                "color:#1f2d48; font-size:12px; background:#eef5ff; border:1px solid #c8d7f2; border-radius:8px; padding:8px;"
            )
            ...
            btn_close.clicked.connect(lambda: self._profile_popup.close() if self._profile_popup is not None else None)
        ...
        if isinstance(info, QLabel):
            info.setText(
                f"Chrome GROK đang mở để đăng nhập.\nCDP: {host}:{port}\nProfile: {profile_dir}\n\n"
    ...
    def _delete_profile(self) -> None:
        p = Path(self._last_profile_dir or self._profile_dir())
        if not p.exists():
        ...
        if QMessageBox.question(
            self,
            "Xác nhận",
            f"Bạn chắc chắn muốn xóa profile GROK?\n{p}",
