...
from src.utils.tts_voices import EDGE_TTS_DEFAULT, TIKTOK_TTS_DEFAULT, get_edge_tts_choices, get_tiktok_tts_choices
from src.utils.voice_profiles import get_voice_choices


AUTO_SCENE_LABEL = "Tự động"
AUTO_STYLE_LABEL = "Tự động nhận diện"
...
class _TTSPreviewWorker(QThread):
    ...
    def run(self) -> None:
        try:
            from src.features.storytelling_exporter import create_tts_preview
...
class AudioPlayerWidget(QWidget):
    def __init__(self, parent=None):
        ...
        try:
            self.player.errorOccurred.connect(lambda *_: self._open_external_fallback())
        except Exception:
            pass
    ...
    def set_source(self, path):
        ...
        self.player.play()
        QTimer.singleShot(1200, self._ensure_playing)

    def _ensure_playing(self):
        from PyQt6.QtMultimedia import QMediaPlayer
        if not self._source_path:
            return
        if self.player.playbackState() != QMediaPlayer.PlaybackState.PlayingState and self.player.position() <= 0:
            self._open_external_fallback()

    def _open_external_fallback(self):
        if self._fallback_opened or not self._source_path:
            return
...
class IdeaToVideoTab(QWidget):
    def __init__(self, config=None, parent: QWidget | None = None):
        ...
        self.storytelling_motion_cb = QCheckBox("Bật chuyển động ảnh nhẹ (Zoom/Pan)")
        self.storytelling_motion_cb.setChecked(bool(getattr(self._cfg, "idea_storytelling_motion", True)))
        self.storytelling_motion_cb.stateChanged.connect(lambda _=0: self._persist_config())
        cfg_layout.addRow("", self.storytelling_motion_cb)

        scene_default = int(getattr(self._cfg, "idea_scene_count", 1) if self._cfg is not None else 1)
        self.scene_count = QComboBox()
        self.scene_count.setEditable(True)
        self.scene_count.addItem(AUTO_SCENE_LABEL, 0)
        for idx in range(1, 101):
            self.scene_count.addItem(str(idx), idx)
        scene_idx = self.scene_count.findData(scene_default)
        self.scene_count.setCurrentIndex(scene_idx if scene_idx >= 0 else 1)
        try:
            self.scene_count.lineEdit().setValidator(QIntValidator(0, 100, self.scene_count))
            self.scene_count.lineEdit().setPlaceholderText(AUTO_SCENE_LABEL)
        except Exception:
            pass
        ...
        if style_default == "Music_Video_Aestheticic":
            style_default = "Music_Video_Aesthetic"
        ...
        cfg_layout.addRow("Kiểu giọng:", self.voice_wrap)
        ...
        try:
            ...
            self.scene_count.currentIndexChanged.connect(lambda _=0: self._persist_config())
            try:
                self.scene_count.lineEdit().editingFinished.connect(self._persist_config)
            except Exception:
                pass
    ...
    def _refresh_voice_choices(self) -> None:
        ...
        if output_mode == "storytelling_image" and provider == "tiktok":
            choices = get_tiktok_tts_choices(locale)
            if choices:
                self._voice_combo_mode = "tiktok"
                default_key = TIKTOK_TTS_DEFAULT
            else:
    ...
    def _preview_tts_voice(self) -> None:
        ...
        preview_provider = "edge" if self._voice_combo_mode == "edge" and provider in {"auto", "tiktok"} else provider
    ...
    def _on_tts_preview_complete(self, ok: bool, path: str, message: str) -> None:
        ...
        QTimer.singleShot(350, lambda p=path: QDesktopServices.openUrl(QUrl.fromLocalFile(p)))
    ...
    def _update_visibility(self) -> None:
        ...
        voice_visible = output_mode == "video" or (is_storytelling and tts_provider != "off" and not is_voicebox)
        ...
        if voice_label:
            ...
            voice_label.setText("Giọng đọc:" if is_storytelling else "Kiểu giọng:")
        ...
        self.btn_preview_voice.setVisible(is_storytelling and tts_provider != "off" and not is_voicebox)
        ...
        self.storytelling_motion_cb.setVisible(is_storytelling)
    ...
    def get_scene_count(self) -> int:
        raw = str(self.scene_count.currentText() or "").strip()
        if not raw or raw.casefold() in {"auto", "tu dong", "tự động", AUTO_SCENE_LABEL.casefold()}:
            return 0
        data = self.scene_count.currentData()
        if data == 0 and raw == AUTO_SCENE_LABEL:
            return 0
        try:
            val = int(raw)
        except Exception:
    ...
    def get_settings(self) -> dict[str, str | int]:
        ...
        return {
            ...
            "idea_storytelling_motion": self.storytelling_motion_cb.isChecked(),
        }
    ...
    def _persist_config(self) -> None:
        ...
        try:
            ...
            setattr(self._cfg, "idea_storytelling_motion", self.storytelling_motion_cb.isChecked())
            setattr(
                self._cfg,
            ...
            elif self._voice_combo_mode in {"edge", "tiktok"}:
                default_voice = TIKTOK_TTS_DEFAULT if self._voice_combo_mode == "tiktok" else EDGE_TTS_DEFAULT
                setattr(self._cfg, "idea_tts_voice", current_voice or default_voice)
