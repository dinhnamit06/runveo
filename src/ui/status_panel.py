...
from src.ui.status_help_view import build_status_help_view, get_status_help_file_path
from src.workflows.A_workflow_text_to_video import TextToVideoWorkflow
from src.workflows.worker_run_workflow_grok import GrokImageToVideoWorker, GrokTextToVideoWorker
from src.core.settings_manager import SettingsManager, WORKFLOWS_DIR, BASE_DIR, DATA_GENERAL_DIR, get_icon_path
from src.core.branding_config import OWNER_ZALO_URL
from src.utils.voice_profiles import VOICE_JSON, VOICE_OPTIONS, get_voice_profile_text, normalize_locale
from src.features.gemini_automation import GeminiAutomation
from src.utils.content_source import build_source_to_video_idea, fetch_url_text, limit_source_text, read_pdf_text
...
class _IdeaToVideoWorker(QThread):
    ...
    def __init__(
        ...
        try:
            self._scene_count = max(0, min(100, int(scene_count)))
        except Exception:
            self._scene_count = 1
    ...
    def run(self) -> None:
        ...
        try:
            from src.features.idea_to_video import idea_to_video_workflow
...
class _StorytellingExportWorker(QThread):
    ...
    def run(self) -> None:
        try:
            from src.features.storytelling_exporter import export_storytelling_video
...
class StatusPanel(QWidget):
    ...
    def _storytelling_target_locale(self) -> str:
        locale = normalize_locale(getattr(self._cfg, "idea_dialogue_language", ""))
        if locale:
            return locale
        voice_key = str(getattr(self._cfg, "idea_tts_voice", "") or "").strip()
        if not voice_key:
            voice_key = str(getattr(self._cfg, "idea_voice_profile", "") or "").strip()
        meta = VOICE_JSON.get(voice_key, {}) if voice_key else {}
        locale = normalize_locale(meta.get("locale"))
        return locale or "vi-VN"

    def _storytelling_target_base_language(self) -> str:
        locale = self._storytelling_target_locale()
        return locale.split("-", 1)[0].lower() if locale else ""

    def _looks_like_storytelling_target_text(self, value: str) -> bool:
        text = self._clean_storytelling_narration_line(value)
        if not text or self._storytelling_target_base_language() != "vi":
            return True
    ...
    def _storytelling_safe_narration_text(self, value: str) -> str:
        text = self._clean_storytelling_narration_line(value)
        if not text:
            return ""
        return text if self._looks_like_storytelling_target_text(text) else ""
    ...
    def _idea_dialogue_lines(self, prompt_obj: dict) -> list[dict]:
        ...
        for line in dialogue:
            if isinstance(line, dict):
                ...
                speaker = "NARRATOR"
                voice = ""
            line_text = self._storytelling_safe_narration_text(line_text)
            if line_text:
    ...
    def _idea_video_prompt_from_prompt_text(
        ...
        summary_narration = self._storytelling_safe_narration_text(summary)
        narration_text = " ".join(line["line"] for line in dialogue_lines).strip()
        ...
        is_vi = self._storytelling_target_base_language() == "vi"
        ...
        if is_vi:
            audio_voice = f"NGƯỜI DẪN CHUYỆN ({voice_hint}, voice-over)" if voice_hint else "NGƯỜI DẪN CHUYỆN (clear natural voice-over)"
        else:
        ...
        if character_lock:
            if is_vi:
                video_parts.append(f"Khóa nhân vật và tính nhất quán: {character_lock}.")
            else:
                video_parts.append(f"Characters and consistency lock: {character_lock}.")
        if background_lock:
            if is_vi:
                video_parts.append(f"Bối cảnh: {background_lock}.")
            else:
        ...
        if narration_for_prompt:
            if is_vi:
                audio_parts.append(f'Âm thanh: {audio_voice} nói "{narration_for_prompt}".')
            else:
                audio_parts.append(f'Audio: {audio_voice} says "{narration_for_prompt}".')
        else:
            if is_vi:
                audio_parts.append("Âm thanh: không có giọng nói.")
            else:
                audio_parts.append("Audio: no voice.")
                
        if foley:
            if is_vi:
                audio_parts.append(f"Hiệu ứng âm thanh (ASMR): {foley}.")
            else:
                audio_parts.append(f"ASMR: {foley}.")
        if music and music.lower() not in {"none", "no", "no music", "silent", "silence", "n/a"}:
            if is_vi:
                audio_parts.append(f"Nhạc nền (BGM): {music}.")
            else:
                audio_parts.append(f"BGM: {music}.")
        else:
            if is_vi:
                audio_parts.append("Nhạc nền (BGM): không có nhạc.")
            else:
    ...
    def _storytelling_item_from_prompt(self, prompt_text: str, index: int) -> dict:
        ...
        dialogue_lines = [item["line"] for item in self._idea_dialogue_lines(prompt_obj)]
        summary = str(prompt_obj.get("summary") or "").strip()
        narration = " ".join(dialogue_lines).strip()
        if not narration:
            narration = self._storytelling_safe_narration_text(summary)
    ...
    def start_idea_to_video(self, idea_settings: dict) -> None:
        ...
        try:
            scene_count = max(0, min(100, int(idea_settings.get("scene_count", 1))))
        except Exception:
            scene_count = 1
        ...
        scene_label = "Tự động" if scene_count <= 0 else str(scene_count)
        self._append_run_log(f"🚀 Khởi động Idea to Video | project={project_name} | scenes={scene_label} | style={style} | output={mode_label}")
    ...
    def _start_generate_image_rows(self, rows: list[int]) -> bool:
        ...
        try:
            from src.workflows.A_workflow_generate_image import GenerateImageWorkflow
    ...
    def _start_generate_image_reference_rows(self, rows: list[int], shared_characters: list[dict] | None = None) -> bool:
        ...
        try:
            from src.workflows.A_workflow_image_to_image import GenerateImageWorkflow
    ...
    def _on_workflow_complete(self) -> None:
        ...
        for wf in list(self._workflows):
            try:
                if wf and wf.isRunning() and wf != self.sender():
                    alive_workflows.append(wf)
            except Exception:
                pass
        self._workflows = alive_workflows
    ...
    def _close_all_workflow_chrome_profiles(self) -> None:
        try:
            from src.api.chrome import kill_profile_chrome as kill_veo_chrome, resolve_profile_dir as resolve_veo_profile

            kill_veo_chrome(resolve_veo_profile())
        except Exception:
            pass
        try:
            from src.api.grok_chrome_manager import kill_profile_chrome as kill_grok_chrome, resolve_profile_dir as resolve_grok_profile
    ...
    def _start_storytelling_export_for_rows(self, rows: list[int]) -> bool:
        ...
        for r in story_rows:
            ...
            raw_narration = str(payload.get("narration") or "").strip()
            narration = self._storytelling_safe_narration_text(raw_narration)
            if raw_narration and not narration:
                self._append_run_log(f"⚠️ Bỏ narration sai ngôn ngữ ở dòng {int(r) + 1}.")
            if not narration:
                prompt_obj = self._parse_storytelling_prompt_object(fallback_text)
                if isinstance(prompt_obj, dict):
                    fallback_summary = str(prompt_obj.get("summary") or "").strip()
                    narration = self._storytelling_safe_narration_text(fallback_summary)
                    if fallback_summary and not narration:
                        self._append_run_log(f"⚠️ Summary fallback của dòng {int(r) + 1} không khớp ngôn ngữ đích, dùng audio im lặng.")
            items.append(
                {
        ...
        if tts_provider not in {"auto", "edge", "sapi", "tiktok", "off"}:
            tts_provider = "auto"
        if tts_provider == "off":
            ...
            if tts_voice:
                voice_key = tts_voice
        aspect_ratio = str(getattr(self._cfg, "video_aspect_ratio", "9:16") or "9:16")
        enable_motion = bool(getattr(self._cfg, "idea_storytelling_motion", True))
    ...
    def _load_merge_video_module(self):
        module_path = Path(__file__).resolve().parent.parent / "features" / "merge+video.py"
