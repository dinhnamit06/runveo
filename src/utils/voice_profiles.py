...
def get_voice_profile_text(key: str | None) -> str:
    meta = get_voice_metadata(key)
    if str(meta.get("style") or "").strip().lower() == "tiktok":
        return ""
...
def _is_tts_only_voice(key: str, meta: dict) -> bool:
    return str(key or "").startswith("tiktok_") or str(meta.get("style") or "").strip().lower() == "tiktok"
...
def get_enabled_voice_keys() -> list[str]:
    keys = [
        key for key in VOICE_OPTIONS
        if _voice_enabled(get_voice_metadata(key)) and not _is_tts_only_voice(key, get_voice_metadata(key))
    ]
...
def get_voice_choices(target_language: str | None = None, include_none: bool = True) -> list[tuple[str, str]]:
    ...
    for key in VOICE_OPTIONS:
        ...
        if not _voice_enabled(meta):
            continue
        if _is_tts_only_voice(key, meta):
            continue
        if key == "None_NoVoice":
            continue
        aliases = _normalized_aliases(meta)
