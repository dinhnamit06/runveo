...
from src.core.settings_manager import DATA_GENERAL_DIR, WORKFLOWS_DIR
from src.ui.style import STYLE_JSON
...
AUTO_STYLE_LABELS = {"auto", "tự động", "tu dong", "tự động nhận diện", "tu dong nhan dien"}
STYLE_ALIASES = {
    "Music_Video_Aestheticic": "Music_Video_Aesthetic",
}

VIETNAMESE_DIACRITICS = "ăâđêôơưáàảãạắằẳẵặấầẩẫậéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵ"
ENGLISH_STOP_WORDS = {
    "a", "an", "and", "are", "as", "at", "by", "coach", "for", "from", "his",
    "in", "into", "is", "of", "on", "or", "our", "team", "their", "the",
    "to", "toward", "with", "without",
}
VIETNAMESE_ASCII_HINTS = {
    "ban", "bong", "cac", "cau", "chien", "chung", "doi", "duoc", "giai",
    "hlv", "khong", "muc", "nam", "nguoi", "nhung", "se", "thang", "tran",
    "tuyen", "va", "viet", "voi",
}


def _clean_language_probe_text(value: str) -> str:
...
def _looks_like_target_language_text(value: str, lang_code: str) -> bool:
    text = _clean_language_probe_text(value)
    base_lang = str(lang_code or "").strip().lower().split("-", 1)[0]
    if not text or base_lang != "vi":
        return True

    lowered = re.sub(r"[^0-9A-Za-zÀ-ỹĂăÂâĐđÊêÔôƠơƯư\s]", " ", text).lower()
    lowered = re.sub(r"\s+", " ", lowered).strip()
    if not lowered:
        return True
    if any(ch in lowered for ch in VIETNAMESE_DIACRITICS):
        return True
    ...
    english_hits = sum(1 for word in words if word in ENGLISH_STOP_WORDS)
    vietnamese_hits = sum(1 for word in words if word in VIETNAMESE_ASCII_HINTS)
    if english_hits >= 2 and english_hits >= vietnamese_hits + 1:
        return False
    return True


def _scene_dialogue_matches_target_language(scene_obj: dict, lang_code: str) -> bool:
    base_lang = str(lang_code or "").strip().lower().split("-", 1)[0]
    if base_lang != "vi" or not isinstance(scene_obj, dict):
        return True
    ...
    for line in dialogue:
        if isinstance(line, dict):
            text = line.get("line") or line.get("text") or line.get("content") or line.get("dialogue") or ""
        else:
            text = line
        if not _looks_like_target_language_text(text, lang_code):
            return False
    return True
...
def _is_auto_style(style: str | None) -> bool:
    text = str(style or "").strip()
    if not text:
        return True
    return text.casefold() in AUTO_STYLE_LABELS


def _normalize_style_key(style: str | None) -> str:
    text = str(style or "").strip()
    text = STYLE_ALIASES.get(text, text)
    if _is_auto_style(text):
        return "Tự động nhận diện"
    if text in STYLE_JSON:
        return text
    return "3d_Pixar"


def _coerce_scene_count(scene_count, default: int = 5) -> int:
    raw = str(scene_count or "").strip()
    if raw.casefold() in {"auto", "tu dong", "tự động"}:
        return 0
    try:
        value = int(scene_count)
    except Exception:
        value = int(default)
    return max(0, min(100, value))


def _fallback_auto_scene_count(idea: str) -> int:
    text = str(idea or "")
...
def _fallback_auto_style(idea: str) -> str:
    text = str(idea or "").casefold()
    if "source kind: báo" in text or "news" in text or "article" in text:
        return "Documentary_style"
    if "source kind: truyện tranh" in text or "comic" in text or "manga" in text:
        return "Comic_Western"
    if any(token in text for token in ("kinh dị", "horror", "ma ", "ghost", "giám sát", "camera an ninh")):
        return "CCTV_Found_Footage"
    if any(token in text for token in ("anime", "nhật", "japan", "manga")):
        return "Anime_Japan"
    if any(token in text for token in ("tin tức", "chính quyền", "pháp luật", "báo cáo", "quy hoạch")):
        return "Documentary_style"
    if any(token in text for token in ("trẻ em", "dễ thương", "cute", "gia đình")):
        return "3d_Pixar"
    return "Live_action_cinematic"


def _guard_auto_style_choice(idea: str, style: str) -> str:
    text = str(idea or "").casefold()
    selected = _normalize_style_key(style)
    news_markers = (
        "source kind: báo",
        "source kind: news",
        "news/article",
    ...
    if any(marker in text for marker in news_markers):
        news_safe = {"Documentary_style", "Realistic", "Hyper_Realistic", "Live_action_cinematic"}
        if selected not in news_safe:
            return "Documentary_style"
    if any(marker in text for marker in comic_markers):
        comic_safe = {"Comic_Western", "Manga_Black_White", "Anime_Japan", "2d_Cartoon", "Stick_Figure"}
        if selected not in comic_safe:
            return "Comic_Western"
    return selected


def resolve_auto_scene_and_style(
    idea: str,
    scene_count,
    style,
    project_dir: str,
    ...
    requested_scene_count = _coerce_scene_count(scene_count, default=5)
    requested_style = _normalize_style_key(style)
    needs_scene = requested_scene_count <= 0
    needs_style = _is_auto_style(requested_style)

    if not needs_scene and not needs_style:
        return requested_scene_count, requested_style
    ...
    fallback_count = _fallback_auto_scene_count(idea) if needs_scene else requested_scene_count
    fallback_style = _fallback_auto_style(idea) if needs_style else requested_style

    api_keys = load_api_keys()
    if not api_keys:
        return max(1, fallback_count), fallback_style

    allowed_styles = sorted(STYLE_JSON.keys())
    style_lines = "\n".join(f"- {key}" for key in allowed_styles)
    fixed_count_note = "Choose scene_count automatically." if needs_scene else f"scene_count is fixed at {requested_scene_count}; return exactly {requested_scene_count}."
    fixed_style_note = "Choose the best style_key from the allowed list." if needs_style else f"style_key is fixed at {requested_style}; return exactly {requested_style}."
    source_excerpt = str(idea or "")[:12000]

    prompt = f"""You are planning a short AI video from a user idea, article, story, comic source, or PDF extract.

Decide ONLY:
1. scene_count: number of scenes. Each scene is about 8 seconds. Use enough scenes for opening, body, and ending, but avoid unnecessary length.
2. style_key: the most suitable output visual style from the allowed list.

Rules:
- {fixed_count_note}
- {fixed_style_note}
- For news/articles, prefer Documentary_style, Realistic, or Live_action_cinematic.
- For text stories, choose based on genre and tone: Live_action_cinematic, Anime_Japan, 3d_Pixar, Noir_Black_and_White, or other fitting styles.
- For comics/manga, choose Comic_Western, Manga_Black_White, Anime_Japan, or a requested conversion style if the idea asks for one.
- For horror/security/crime footage, consider CCTV_Found_Footage or Noir_Black_and_White.
- For cute/family/children/fable stories, consider 3d_Pixar, 2d_Cartoon, or 3d_Cartoon.
- scene_count should usually be 4-12. Use up to 20 only if the source is long and has many clear story beats.
- Return ONLY valid JSON, no markdown.

Allowed style_key values:
{style_lines}
...
Output JSON schema:
{{"scene_count": 6, "style_key": "Documentary_style", "reason": "short reason in Vietnamese"}}"""

    log("🧠 AI đang tự chọn số cảnh/phong cách phù hợp...")
    ...
    parsed = parse_json_response(response_text or "")
    if not isinstance(parsed, dict):
        log(f"⚠️ AI auto không trả JSON hợp lệ; dùng fallback scenes={fallback_count}, style={fallback_style}")
        return max(1, fallback_count), fallback_style

    final_count = fallback_count
    if needs_scene:
        try:
            final_count = int(parsed.get("scene_count") or fallback_count)
        except Exception:
            final_count = fallback_count
    final_count = max(1, min(100, final_count))

    final_style = fallback_style
    if needs_style:
        candidate = _normalize_style_key(parsed.get("style_key") or parsed.get("style") or fallback_style)
        if candidate in STYLE_JSON:
            final_style = candidate
        final_style = _guard_auto_style_choice(idea, final_style)
    reason = str(parsed.get("reason") or "").strip()
    suffix = f" | lý do: {reason}" if reason else ""
    log(f"✅ AI auto chọn: {final_count} cảnh, style={final_style}{suffix}")
    return final_count, final_style
...
def gemini_step_1(script, project_dir, scene_count, style, log_callback=None, stop_check=None):
    ...
    num_scenes = _coerce_scene_count(scene_count, default=5) or 5
    style = _normalize_style_key(style)
...
- Speaker: CHAR_ID, character name, or "NARRATOR" for voice-over
- Language: {lang_code}
- Line: Dialogue ONLY in {language_str}, in brackets like [Hôm nay là ngày tốt lành!]
- IMPORTANT: If no character is speaking but the scene needs narration, you MUST add a "NARRATOR" line in {language_str}.
    ...
    for chunk_idx, chunk_scene_ids in enumerate(chunks, 1):
        ...
        while pending_scene_ids and attempt < max_attempts_per_chunk:
            ...
            for obj in parsed_objects:
                ...
                normalized_obj = normalize_scene_for_step3(obj)
                if not _scene_dialogue_matches_target_language(normalized_obj, lang_code):
                    log(f"⚠️ Step2 chunk {chunk_idx}, scene {scene_id}: dialogue không khớp ngôn ngữ đích ({lang_code})")
                    continue
                if scene_id in pending_scene_ids and scene_id not in output_map:
                    output_map[scene_id] = normalized_obj
...
📝 Dialogue and Narration: All spoken text MUST be in {language_str} ({lang_code}). If there is no character dialogue, ensure there is a NARRATOR line in the 'dialogue' array in {language_str}.
        ...
        for idx, prompt_line in enumerate(chunk_output_lines, 1):
            ...
            try:
                ...
                if scene_id not in expected_scene_ids:
                    ...
                    continue
                if not _scene_dialogue_matches_target_language(prompt_obj, lang_code):
                    log(f"⚠️ Chunk {chunk_idx}, Prompt {idx}: dialogue không khớp ngôn ngữ đích ({lang_code})")
                    continue
                        ...
                        for retry_prompt_line in retry_prompts:
                            ...
                            try:
                                ...
                                if all(f in retry_obj for f in required_fields) and retry_scene_id in missing_scene_ids:
                                    if not _scene_dialogue_matches_target_language(retry_obj, lang_code):
                                        log(f"⚠️ Retry chunk {chunk_idx}, scene {retry_scene_id}: dialogue không khớp ngôn ngữ đích ({lang_code})")
                                        continue
...
def idea_to_video_workflow(
    ...
    try:
        ...
        num_scenes, style = resolve_auto_scene_and_style(
            idea,
            scene_count,
            style,
            project_dir,
