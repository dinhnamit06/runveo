...
async def api_image_to_video_in_page(
    ...
    file_uris: list[str],
    parent_post_ids: list[str] | None,
    statsig_headers: dict,
    cfg: ImageToVideoConfig,
    timeout_seconds: int,
    job_index: int = 0,
) -> dict:
    """Generate video from uploaded image(s).
    ...
    for uri in (file_uris or []):
        ...
        if raw_uri.startswith("http://") or raw_uri.startswith("https://"):
            asset_urls.append(raw_uri)
        elif raw_uri:
            asset_urls.append(f"https://assets.grok.com/{raw_uri.lstrip('/')}")

    prompt_text = str(prompt or "").strip()
    # Message format: joined asset urls + space + prompt
    message_prefix = " ".join(asset_urls).strip()
    message = f"{message_prefix}  {prompt_text}".strip() if message_prefix else prompt_text

    # We use the list of attachments
    attachments = []
    if parent_post_ids:
        attachments.extend([str(pid or "").strip() for pid in parent_post_ids if str(pid or "").strip()])
    
    # Plus any metadata IDs not already in parents
    seen = set(attachments)
    for mid in (file_metadata_ids or []):
        smid = str(mid or "").strip()
        if smid and smid not in seen:
            attachments.append(smid)
            seen.add(smid)

    if not attachments:
        return {"status": 0, "error": "No attachments provided", "lastEvent": None}

    # For parentPostId in modelConfigOverride, we usually pick the first one
    primary_parent = attachments[0]
    ...
    result = await page.evaluate(
        """(async ({ message, attachments, primaryParent, cfg, statsigHeaders, timeoutSeconds, jobIndex }) => {
          ...
          log('conversation_start', { attachments, hasMessage: !!message, messagePreview: String(message || '').slice(0, 120) });
          ...
          const convoPayload = {
            ...
            responseMetadata: {
              ...
              modelConfigOverride: {
                modelMap: {
                  videoGenModelConfig: Object.assign({ parentPostId: primaryParent }, cfg),
                },
              },
            },
          };
    ...
    first_uri = file_uris[0] if file_uris else ""
    user_id = str(_extract_user_id_from_file_uri(first_uri) or stream_user_id).strip()
...
async def api_run_image_to_video_job(
    ...
    1. Upload image(s)
    2. Create posts for each image
    3. Generate video from attachments
    4. Upscale to HD
    ...
    # Step 1 & 2: Upload images and create posts
    for i, path in enumerate(image_paths):
        upload_result = await api_upload_image_in_page(page, path, statsig_headers)
        if not upload_result.get("fileMetadataId"):
            return {
                "prompt": prompt,
                "imagePaths": [str(p) for p in image_paths],
        ...
        mid = upload_result["fileMetadataId"]
        uri = upload_result.get("fileUri", "")
        file_metadata_ids.append(mid)
        file_uris.append(uri)
        
        upload_media_url = _normalize_assets_url(uri, add_download_query=False)
        ...
        pid = str(create_post_result.get("postId") or "").strip()
        if pid:
            parent_post_ids.append(pid)
            # If the post link returned a better media URL, use it
            murl = str(create_post_result.get("mediaUrl") or "").strip()
            if murl:
                file_uris[-1] = murl
    ...
    return {
        "prompt": prompt,
        "imagePaths": [str(p) for p in image_paths],
        "fileMetadataIds": file_metadata_ids,
        "fileUris": file_uris,
        "parentPostIds": parent_post_ids,
